...
namespace TaskDMD.Handlers
{
     public class PreRecordMessageSettingHandler : IDMDMessageHandler
    {
        public void Handle(ASI.Wanda.DMD.Message.Message message, TaskDMDHelper<ASI.Wanda.DMD.DMD_API> helper)
        {
            ...
            var PreRecordMessageSetting = new ASI.Wanda.DMD.JsonObject.DCU.FromDMD.PreRecordMessageSetting(ASI.Wanda.DMD.Enum.Station.OCC)
            {
                seatID = obj.seatID,
                msg_id = obj.msg_id,
            ...
            helper.UpdataDCUPreRecordMessage();
            SendToAllTasks(helper, PreRecordMessageSetting);
        }
